...
export default function useApp() {
  const { dropdown, pagination, deletePhotos, photoList } = useAppContext();
  ...
  const handleLoadMore = useCallback(async () => {
    try {
      const newData: PhotoType.PhotoListResponses = await PhotoRepository.getAllPhoto(
        "/photos/list",
        {
  ...
  return {
    photoList,
    handleLoadMore,
    isLastPage: pagination.state.isLastPage,
    isDeleteState: deletePhotos.photos.length > 0,
  };
}
