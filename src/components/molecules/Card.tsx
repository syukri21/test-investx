...
const Card = ({ data, isDeleteState }: Props) => {
    ...
    return (
        <div
            className={`${isDeleteState && !isSelected ? "opacity-50" : "opacity-100"} flex flex-col justify-center items-center cursor-pointer`}
            onClick={handleToggleSelect}
        >
            <div className='relative'>
                <img src={data.raw} className='h-40 rounded-sm w-full object-cover' alt='#' />
                {isSelected && <FontAwesomeIcon icon={faCheckSquare} className='text-red-100 absolute top-0 ml-1 mt-1 left-0'></FontAwesomeIcon>}
            </div>
